...
import com.cts.fms.repository.EventRepository;
...
public class EventServiceImpl implements EventService {
	...
	private static String[] columns = { "event_id", "base_location", "beneficiary_name", "council_name", "event_name",
			...
			"lives_impacted", "business_unit", "event_status", "iiep_category","totalVolunteers","totalParticipants" };
	...
	public Mono<String> SendMail() {
		...
		Authenticator auth = new Authenticator() { // JAVAX.MAIL.AUTHENTICATIONFAILEDEXCEPTION: 535-5.7.8 USERNAME AND
			...
			protected PasswordAuthentication getPasswordAuthentication() {
				return new PasswordAuthentication("naicey.001@gmail.com", "password");
			}
		};
		Session session = Session.getInstance(props, auth);
		send(session, "dibya90@gmail.com", "Subject host", "Mail sent from javax");
	...
	public static void send(Session session, String toEmail, String subject, String body) {
		try {
			...
			msg.setFrom(new InternetAddress("naicey.001@gmail.com"));
	...
	private void copyToExcel(List<Event> output) {
		...
		for (Event eve : output) {
			...
			row.createCell(15).setCellValue(eve.getTotalVolunteers());
			
			row.createCell(16).setCellValue(eve.getTotalParticipants());
		}
